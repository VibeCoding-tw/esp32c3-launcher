...
void motorRampTask() {
    ...
    if (targetSpeedT == 0) {
        ...
        if (currentSpeedT == 0) {
            // 使用 Kick 參數
            if (targetSpeedT > 0) {
                currentSpeedT = motorConfig.pwmStartKickT;
            } else {
                currentSpeedT = -motorConfig.pwmStartKickT;
            }
        }
        ...
        // 使用 Step 參數
        if (abs(targetSpeedT - currentSpeedT) > motorConfig.rampAccelStepT) {
            if (targetSpeedT > currentSpeedT) {
                currentSpeedT += motorConfig.rampAccelStepT;
            } else {
                currentSpeedT -= motorConfig.rampAccelStepT;
            }
        } else {
    ...
    if (targetSpeedS == 0) {
        ...
        if (currentSpeedS == 0) {
             // 使用 Kick 參數
            if (targetSpeedS > 0) {
                currentSpeedS = motorConfig.pwmStartKickS;
            } else {
                currentSpeedS = -motorConfig.pwmStartKickS;
            }
        }
        ...
        // 使用 Step 參數
        if (abs(targetSpeedS - currentSpeedS) > motorConfig.rampAccelStepS) {
             if (targetSpeedS > currentSpeedS) {
                currentSpeedS += motorConfig.rampAccelStepS;
            } else {
                currentSpeedS -= motorConfig.rampAccelStepS;
            }
        } else {
