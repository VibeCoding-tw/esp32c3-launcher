// 包含必要的庫
#include <WiFi.h>
#include <WebServer.h>               // *** 替換為同步 Web Server 庫 ***
#include <WiFiManager.h>             // *** 替換為同步 WiFiManager 庫 ***
...
// bool isConfigurationMode = false;   // 同步版 WiFiManager 中通常不需要這個旗標

// *** 替換為同步 Web Server ***
WebServer server(80);                // 實例化同步 Web Server
WiFiManager wm;                      // 實例化同步 WiFiManager
...
void setupWebServer() {
    Serial.println("--- 啟動 Web Server (同步) ---");
    ...
    server.begin();
    Serial.println("HTTP 伺服器已啟動於 Port 80 (同步)。");
}
...
// --- 連線或啟動 Wi-Fi 配置入口網站 (同步版) ---
void connectToWiFi() {

    WiFi.setHostname(globalHostname.c_str());
    
    // *** 移除 setAPCallback 裡的 setupWebServer() 呼叫 ***
    wm.setAPCallback([](WiFiManager *myWiFiManager) {
        ...
        // 【重要】這裡不再呼叫 setupWebServer(); 讓 WiFiManager 自己的 Portal 運行。
    });
    ...
    // *** 這裡會阻塞，直到連線成功或進入 Portal 模式 ***
    if (!wm.autoConnect("ESP32-Setup")) {        
        Serial.println("AutoConnect 失敗，裝置將持續保持在配置模式 (Portal Running)。");
        // 如果失敗，此時程式碼不會立即到這裡，它會被 wm 內部阻塞。
        // 一旦用戶退出 Portal，wm.autoConnect 才會返回 false。
    } else {
...
void connectToWiFi() {
    ...
    // *** 同步阻塞調用，直到連線成功或進入配置模式 ***
    if (!wm.autoConnect("ESP32-Setup")) {        
        Serial.println("AutoConnect 失敗，裝置將持續保持在配置模式。");
        // 進入阻塞迴圈，僅處理 Portal 請求 (由 setupWebServer 處理)
    } else {
        Serial.println("✅ Wi-Fi 連線成功!");
    }
...
void setup() {
    ...
    // 1. 連線 Wi-Fi (WiFiManager 的 autoConnect)
    connectToWiFi();

    // 只有在成功連線到 STA 模式下才需要配置 mDNS 和 OTA，並啟動 Web Server
    if (WiFi.status() == WL_CONNECTED) {
        ...
        Serial.println("⚠️ 進入 AP 配置模式。WiFiManager Portal 已啟動，Web Server 已在 AP Callback 中啟動。");
    }
}

// --- Loop ---
void loop() {
    // 【重要】只在 STA 模式（連線成功）時才處理您的 Web Server 請求
    if (WiFi.status() == WL_CONNECTED) {
        server.handleClient(); 
        ArduinoOTA.handle();
    } 
    // 否則，在 AP 模式下，WiFiManager 會處理 server.handleClient()。
